...
/datum/sword_tech/pyrokinesis/use_traverse_ability()
	...
	var/turf/ending = get_ranged_target_turf(new_soul, new_soul.dir, final_traverse_range)
	var/list/affected_turfs = list()
	var/datum/reagent/chemical = GLOB.chemical_reagents_list["utnapthal"]

	for(var/turf/T in get_line(new_soul, ending))
		if(T == ending)
			continue
		affected_turfs += T
	...
	new_soul.throw_atom(get_edge_target_turf(new_soul, new_soul.dir), final_traverse_range, SPEED_VERY_FAST, src, FALSE)
